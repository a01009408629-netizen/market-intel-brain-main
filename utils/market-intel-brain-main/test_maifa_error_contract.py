    ...
    try:
        ...
        for source, result in fetch_results.items():
            ...
            if missing_fields:
                ...
                print(f"  ✅ TITANIUM CONTRACT COMPLIANT")
        ...
        for stage_results in all_results:
            for source, result in stage_results.items():
                if isinstance(result, Exception) and not isinstance(result, MAIFAError):
                    ...
                    print(f"❌ RAW EXCEPTION LEAKED: {source} -> {type(result).__name__}")
        
        if raw_exceptions_found == 0:
            print("✅ ZERO RAW EXCEPTIONS LEAKED")
            ...
            for source, result in stage_results.items():
                if result.get("status") == "error":
                    ...
                    if missing_fields:
                        ...
                        print(f"❌ INCONSISTENT SHAPE: {source} missing {missing_fields}")
        
        if inconsistent_shapes == 0:
            print("✅ ZERO INCONSISTENT ERROR SHAPES")
