"""
MAIFA v3 Market Stack Normalizer
Normalizes validated Market Stack data to standard format
"""
...
class MarketStackNormalizer(DataNormalizer):
    """Market Stack data normalizer"""
    
    def __init__(self):
        self.logger = logging.getLogger("MarketStackNormalizer")
        
    async def normalize(self, validated_data: Dict[str, Any]) -> Dict[str, Any]:
        """Normalize validated Market Stack data to standard format"""
        try:
            ...
            for point in data_points:
                normalized_point = {
                    ...
                    "date": point.get("date"),
                    "time": point.get("time"),
                    "datetime": f"{point.get('date')} {point.get('time', '')}".strip(),
                    "open": point.get("open"),
                    "high": point.get("high"),
                    "low": point.get("low"),
            ...
            if normalized_data["data"]:
                ...
                if closes:
                    normalized_data["summary"] = {
                        ...
                        "symbols": list(set(p.get("symbol") for p in normalized_data["data"] if p.get("symbol"))),
            ...
            self.logger.info(f"Normalized {len(normalized_data['data'])} market data points")
