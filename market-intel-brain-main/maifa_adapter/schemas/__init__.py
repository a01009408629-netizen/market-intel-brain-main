...
class PriceData(BaseMarketData):
    ...
    @validator('price', 'open', 'high', 'low', 'close', 'change', 'change_percent')
    @classmethod
...
class ForexData(BaseMarketData):
    ...
    @validator('exchange_rate', 'bid', 'ask', 'change', 'change_percent')
    @classmethod
    def validate_decimals(cls, v):
        """Ensure financial precision."""
        if v is not None:
    ...
    @classmethod
    def create_pair(cls, values):
        """Create currency pair."""
        if 'from_symbol' in values and 'to_symbol' in values:
            values['symbol'] = f"{values['from_symbol']}{values['to_symbol']}"
        return values
...
class CryptoData(PriceData):
    ...
    @validator('market_cap', 'volume_24h', 'circulating_supply')
    @classmethod
    def validate_decimals(cls, v):
        """Ensure financial precision."""
        if v is not None:
...
class EconomicIndicator(BaseModel):
    ...
    @validator('value', 'change', 'change_percent', 'previous_value')
    @classmethod
    def validate_decimals(cls, v):
        """Ensure financial precision."""
        if v is not None:
...
class CompanyProfile(BaseModel):
    ...
    @validator('symbol', 'country', 'currency')
    @classmethod
